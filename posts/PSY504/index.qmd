...
```{r}
#| output: true
lingdist_msd <- data %>%     # Mean, sd, se, CI_lower, CI_upper
  dplyr::select(ID, Condition, psychdist) %>%
  na.omit() %>%
  group_by(Condition) %>%
  summarize(mean = mean(psychdist),
            sd = sd(psychdist),
            se = sd / sqrt(n()),
...
ggplot(lingdist_msd, aes(x=Condition, y=mean)) + 
  ...
  ylab("Mean Linguistic Distance") + xlab("") +
...
affect_change_msd <- data %>%
  ...
  rename(Change = affect_change) %>%    # Mean, sd, se, CI_lower, CI_upper
  group_by(Condition) %>%
